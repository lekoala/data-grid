...
test.skipIf(IS_WINDOWS)("boots the component and renders rows", async () => {
    await using v = view();
    await v.navigate(`${ensureServer()}/${FIXTURE}`);
    await waitFor(v, "window.grid && window.grid.rows.length > 0");
    expect(await read(v, "window.grid.querySelectorAll('tbody tr').length")).toBe(10);
    expect(await read(v, "window.grid.getSelectionState().mode")).toBe("explicit");
});

test.skipIf(IS_WINDOWS)("server grid paginates through a real click", async () => {
    await using v = view();
    await v.navigate(`${ensureServer()}/${FIXTURE}`);
...
test.skipIf(IS_WINDOWS)("sorting the server grid updates the query and rows", async () => {
    await using v = view();
    await v.navigate(`${ensureServer()}/${FIXTURE}`);
    await waitFor(v, "window.sgrid && window.sgrid.rows.length > 0");
...
test.skipIf(IS_WINDOWS)("typing in a filter narrows the server grid", async () => {
    await using v = view();
    await v.navigate(`${ensureServer()}/${FIXTURE}`);
    await waitFor(v, "window.sgrid && window.sgrid.rows.length > 0");
...
test.skipIf(IS_WINDOWS)("a failing endpoint sets data-error without data-empty", async () => {
    await using v = view();
    await v.navigate(`${ensureServer()}/${FIXTURE}`);
...
test.skipIf(IS_WINDOWS)("reconnecting the element reloads its data", async () => {
    await using v = view();
    await v.navigate(`${ensureServer()}/${FIXTURE}`);
    await waitFor(v, "window.grid && window.grid.rows.length > 0");
...
test.skipIf(IS_WINDOWS)("a rapid double setQuery keeps the last request (abort-race)", async () => {
    await using v = view();
    await v.navigate(`${ensureServer()}/${FIXTURE}`);
    await waitFor(v, "window.sgrid && window.sgrid.rows.length > 0");
